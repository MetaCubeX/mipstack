...
func (b *stackBridge) run(source, destination *Stack, countClient bool) {
	...
	for {
		sizes := []int{0}
		if _, err := source.Read([][]byte{buffer}, sizes, 0); err != nil {
			return
		}
		if countClient {
			b.mu.Lock()
		...
		_, _ = destination.Write([][]byte{buffer[:sizes[0]]}, 0)
	}
}
...
func (l *testPacketLink) run() {
	...
	for {
		...
		_ = l.handlePacket(buffer[:sizes[0]])
	}
}

// handlePacket captures control traffic and emulates configured peer behavior.
func (l *testPacketLink) handlePacket(packet []byte) error {
...
func (l *testPacketLink) handleTCP(packet ipPacket) error {
	...
	for _, response := range responses {
		if err := l.deliverTCP(serverPort, clientPort, response.sequence, acknowledgement, tcpFlagACK|tcpFlagPSH, 65535, nil, response.payload); err != nil {
			return err
		}
	}
	return nil
